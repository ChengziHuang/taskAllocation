...
bool local_search(vector<heavy_task*> taskset, vector<int> index_HI, vector<int> index_LO, vector<double>& response_time_result, int count_max) {
	...
	while (count < count_max) {
		int rule = generate_random_number(1, 8);
		if (rule == 1) {
			// HI_LO to HI_LO
			if (index_HI.size() <= 1) {
				continue;
				...
				if (i == j || taskset[index_HI[i]]->cluster_little->size() == 0) {
					continue;
				}
				else {
					//cout << rule << endl;
					...
					processor target = *select_randomly(taskset[index_i]->cluster_little->processor_list.begin(), taskset[index_i]->cluster_little->processor_list.end());

					heavy_task* new_task_i = new heavy_task(taskset[index_i]);
					heavy_task* new_task_j = new heavy_task(taskset[index_j]);


					new_task_i->cluster_little->del(target);
					new_task_j->cluster_little->add(target);
		...
		if (rule == 2) {
			...
			if (index_HI.size() <= 1) {
				...
				if (i == j || taskset[index_HI[i]]->cluster_delta->size() == 0) {
					continue;
				}
				else {
					//cout << rule << endl;
					...
					processor target = *select_randomly(taskset[index_i]->cluster_delta->processor_list.begin(), taskset[index_i]->cluster_delta->processor_list.end());

					heavy_task* new_task_i = new heavy_task(taskset[index_i]);
					heavy_task* new_task_j = new heavy_task(taskset[index_j]);

					new_task_i->cluster_delta->del(target);
					new_task_j->cluster_delta->add(target);
		...
		if (rule == 3) {
			...
			if (index_LO.size() <= 1) {
				...
				if (i == j || taskset[index_LO[i]]->cluster_little->size() == 0) {
					continue;
				}
				else {
					//cout << rule << endl;
		...
		if (rule == 4) {
			...
			if (index_LO.size() <= 1) {
				...
				if (i == j || taskset[index_LO[i]]->cluster_delta->size() == 0) {
					continue;
				}
				else {
					//cout << rule << endl;
		...
		if (rule == 7) {
			...
			if (index_LO.size() == 0 || index_HI.size() == 0) {
				...
				if (taskset[index_HI[i]]->cluster_little->size() == 0) {
					continue;
				}
				else {
					//cout << rule << endl;
					...
					if (add_index == index_i) {
						new_task_i->cluster_little->del(target);
						new_task_j->cluster_delta->add(target);
						new_task_i->cluster_delta->add(target);
						...
						new_task_i->cluster_little->del(target);
						new_task_j->cluster_delta->add(target);
						new_task_add->cluster_delta->add(target);
		...
		if (rule == 8) {
			...
			if (index_LO.size() == 0 || index_HI.size() == 0) {
				...
				if (taskset[index_LO[i]]->cluster_delta->size() == 0) {
					continue;
				}
				else {
					//cout << rule << endl;
					...
					for (int i = 0; i < index_HI.size(); i++) {
						...
						if (taskset[index_HI[i]]->cluster_delta->find(target) == true) {
					...
					if (del_index == index_j) {
						new_task_i->cluster_delta->del(target);
						new_task_j->cluster_little->add(target);
						new_task_j->cluster_delta->del(target);
						...
						new_task_i->cluster_delta->del(target);
						new_task_j->cluster_little->add(target);
						new_task_del->cluster_delta->del(target);
...
bool allocate_heavy_task(vector<processor> processorset, vector<heavy_task*> taskset) {
	...
	for (int i = 0; i < taskset.size(); i++) {
		while (j < processorset.size()) {
			...
			//taskset[i]->cluster_little->add(processorset[j]);
			
			if (taskset[i]->criticality == "LOW") {
				taskset[i]->cluster_delta->add(processorset[j]);

				if (index_HI.size() != 0) {
					int random_index = index_HI[generate_random_number(0, index_HI.size() - 1)];
					taskset[random_index]->cluster_delta->add(processorset[j]);
				}
			}
			else {
				taskset[i]->cluster_little->add(processorset[j]);
