...
tuple<vector<processor>, vector<task*>> read(string pathFile) {
	...
	double elasic_degree;
	for (int i = 0; i < number_task; ++i) {
		infile >> name >> criticality >> deadline >> C_N >> C_O >> L_N >> L_O >> elasic_degree;
...
tuple<vector<processor>, vector<heavy_task*>> get_remain(vector<processor> processorset, vector<task*> taskset, vector<uniprocessor*> p_success) {
	if (p_success.size() == 0) {
		...
		for (int i = 0; i < taskset.size(); i++) {
			heavyset.push_back(new heavy_task(taskset[i]->name, taskset[i]->criticality, taskset[i]->deadline, taskset[i]->C_N, taskset[i]->C_O, taskset[i]->L_N, taskset[i]->L_O));
		}
	...
	for (int j = 0; j < taskset.size(); j++) {
		if (del_t_index.find(j) == del_t_index.end()) {
			...
			taskset_remain.push_back(new heavy_task(taskset[j]->name, taskset[j]->criticality, taskset[j]->deadline, taskset[j]->C_N, taskset[j]->C_O, taskset[j]->L_N, taskset[j]->L_O));
		}
	}
